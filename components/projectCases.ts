...
export const projectCases: ProjectCase[] = [
  ...
  },
  {
    ...
    summary: '얼굴·음성·설문으로 뇌졸중 위험 신호를 확인하고, 결과와 병원 탐색으로 이어지는 모바일 앱입니다.',
    role: '팀 프로젝트. 서비스 기획·UI/UX, React Native 화면과 카메라·음성·지도·차트 연동을 맡았습니다.',
    mechanism: '민감한 입력을 필요한 파생 결과로만 남긴 과정',
    steps: [
      { label: '문제 상황', state: '민감 입력', title: '얼굴·음성 원본은 분석 뒤 남기지 않아야 했습니다.', body: 'AI 분석에는 원본이 필요하지만, 서비스 저장소에는 남길 이유가 없었습니다.' },
      { label: '제가 판단한 기준', state: '최소 수집', title: 'AI 결과만 남기고 원본은 저장하지 않습니다.', body: '원본은 요청 중에만 분석 서비스로 전달하고, 판정값과 확률만 보존했습니다.' },
      { label: '제가 구현한 방식', state: '입력 경계', title: 'AI 호출 전 파일 형식과 크기를 확인했습니다.', body: '빈 파일·형식·크기를 먼저 검사하고, 계약 범위의 AI 응답만 저장했습니다.' },
      { label: '확인한 결과', state: '결과 안내', title: '결과 뒤에는 가까운 병원 탐색으로 연결했습니다.', body: '자가 확인 결과를 의료 확정 진단으로 표현하지 않고 다음 행동을 안내했습니다.' },
    ],
    result: '원본 비저장 · AI 결과와 병원 탐색 연결',
    conditions: '로컬 테스트 기준입니다. 실제 AI 서버의 보존 정책은 별도 확인이 필요합니다.',
    source: '#dasibom-case-01',
    sourceLabel: '진단 미디어 최소수집 경계',
    supporting: [
      { title: '결과는 사용자 기록과 병원 탐색으로 연결했습니다.', problem: '자가 확인 결과는 안전한 안내와 다음 행동으로 이어져야 했습니다.', body: '진단 상세는 사용자 ID와 함께 조회하고, 결과 화면에서 가까운 병원을 찾을 수 있게 했습니다.', proof: '소유자 조회와 안전한 오류 응답을 테스트 범위에서 확인했습니다.' },
    ],
  },
];
